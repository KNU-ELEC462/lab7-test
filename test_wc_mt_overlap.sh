...
echo "Warming up $TARGET1 (baseline)..."
taskset -c 0-1 "$BIN1" "$INPUT_LARGE" "$NUM_THREADS" > /dev/null 2>&1
...
echo "Warming up $TARGET2 (overlap)..."
taskset -c 0-1 "$BIN2" "$INPUT_LARGE" "$NUM_THREADS" > /dev/null 2>&1
